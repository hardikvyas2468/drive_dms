...
export async function uploadDriveEntity(file, team, doc_name) {
  ...
  while (chunkByteOffset < file.size) {
    ...
    if (chunkIndex === totalChunks - 1) {
      ...
      return `/api/method/dms.api.embed.get_file_content?embed_name=${data.message.name}&parent_entity_name=${doc_name}`
    }
...
async function uploadChunk(
  ...
  const response = await fetch(
    window.location.origin + "/api/method/dms.api.files.upload_file",
    {
      method: "POST",
      body: formData,
      headers: {
