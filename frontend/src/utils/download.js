...
async function getPdfFromDoc(entity_name) {
  const res = await fetch(
    `/api/method/dms.api.files.get_file_content?entity_name=${entity_name}`
  )
...
export function entitiesDownload(team, entities) {
  if (entities.length === 1) {
    if (entities[0].mime_type === "frappe_doc") {
      return fetch(
        `/api/method/dms.api.files.get_file_content?entity_name=${entities[0].name}`
      ).then(async (data) => {
    ...
    return entities[0].is_group
      ...
      : (window.location.href = `/api/method/dms.api.files.get_file_content?entity_name=${entities[0].name}&trigger_download=1`)
  }
  ...
  Promise.all(promises)
    ...
    .then(async function (content) {
      ...
      downloadLink.download = "dms Download " + +new Date() + ".zip"
...
function get_file_content(entity_name) {
  const fileUrl =
    "/api/method/" +
    `dms.api.files.get_file_content?entity_name=${entity_name}`
...
function get_children(team, entity_name) {
  const url =
    "/api/method/" +
    `dms.api.list.files?team=${team}&entity_name=${entity_name}`
  return fetch(url, {
    method: "GET",
    headers: {
