...
const domainTeams = createResource({
  url: "dms.api.product.get_domain_teams",
  auto: true,
  params: {
...
const createPersonalTeam = createResource({
  url: "dms.api.product.create_personal_team",
  makeParams: () => ({
  ...
  onSuccess: (data) => {
    if (data) {
      window.location.replace("/dms/t/" + data)
    } else {
...
const requestInvite = createResource({
  url: "dms.api.product.request_invite",
  onSuccess: () => {
    window.location.replace("/dms/teams")
  },
})
</script>
