                ...
                <Tooltip
                  :text="
                    invite.status === 'Proposed'
                      ? 'A person from your domain has joined dms.'
...
const isAdmin = createResource({
  url: "dms.api.permissions.is_admin",
  params: { team: team.value },
  auto: true,
})

const inviteUsers = createResource({
  url: "dms.api.product.invite_users",
  onSuccess: () => {
    invites.fetch()
    toast("Invite sent!")
  },
...
const invites = createResource({
  url: "dms.api.product.get_team_invites",
  auto: true,
  params: { team: team.value },
})

const removeUser = createResource({
  url: "dms.api.product.remove_user",
})

const updateUserAccess = createResource({
  url: "dms.api.product.set_user_access",
})
</script>
