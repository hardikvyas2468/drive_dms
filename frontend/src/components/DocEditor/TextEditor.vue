...
export default {
  ...
  mounted() {
    ...
    this.$realtime.doc_subscribe("dms File", this.entityName)
    this.$realtime.doc_open("dms File", this.entityName)
  ...
  beforeUnmount() {
    ...
    this.$realtime.doc_close("dms File", this.entityName)
    this.$realtime.doc_unsubscribe("dms File", this.entityName)
    ...
    async wordToHTML() {
      ...
      if (
        ...
        const res = await fetch(
          `/api/method/dms.api.files.get_file_content?entity_name=${ctx.pickedFile.name}`,
          {
            method: "GET",
            headers,
          }
