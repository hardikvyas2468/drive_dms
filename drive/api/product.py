...
from dms.api.permissions import is_admin
...
def get_domain_teams(domain):
    return frappe.db.get_all(
        "dms Team", filters={"team_domain": ["like", "%" + domain]}, fields=["name", "title"]
    )
...
def create_personal_team(email=frappe.session.user, team_name=None):
    ...
    team = frappe.get_doc(
        {
            "doctype": "dms Team",
            "title": team_name if team_name else "Your dms",
...
def request_invite(team, email=None):
    invite = frappe.new_doc("dms User Invitation")
...
def get_invites(email):
    ...
    for i in invites:
        i["team_name"] = frappe.db.get_value("dms Team", i["team"], "title")
    return invites
...
def signup(account_request, first_name, last_name=None, team=None):
    ...
    if account_request.invite:
        invite = frappe.get_doc("dms User Invitation", account_request.invite)
        invite.status = "Accepted"
        invite.save(ignore_permissions=True)
        # Add to that team
        team = frappe.get_doc("dms Team", invite.team)
            ...
            out.append(
                {
                    ...
                    "auth_url": get_oauth2_authorize_url(provider.name, "/dms"),
                    "icon": icon,
                }
            )
    return out
...
def get_settings():
    if frappe.session.user == "Guest":
        return {}
    try:
        return frappe.get_cached_doc("dms Settings", frappe.session.user)
    except:
        return {}
...
def set_settings(updates):
    try:
        settings = frappe.get_doc("dms Settings", frappe.session.user)
    except:
        settings = frappe.get_doc({"doctype": "dms Settings", "user": frappe.session.user})
        settings.insert()
...
def set_user_access(team, user_id, access_level):
    ...
    drive_team = {k.user: k for k in frappe.get_doc("dms Team", team).users}
...
def remove_user(team, user_id):
    ...
    frappe.delete_doc("dms Team Member", drive_team[user_id].name)


@frappe.whitelist()
def get_all_users(team):
    team_users = {k.user: k.access_level for k in frappe.get_doc("dms Team", team).users}
...
def accept_invite(key, redirect=True):
    try:
        invitation = frappe.get_doc("dms User Invitation", key)
    except:
