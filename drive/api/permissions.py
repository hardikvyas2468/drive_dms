...
from dms.utils.users import mark_as_viewed
from dms.utils.files import get_valid_breadcrumbs, generate_upward_path, get_file_type
...
def get_user_access(entity, user=frappe.session.user):
    ...
    if isinstance(entity, str):
        entity = frappe.get_doc("dms File", entity)
...
def get_teams(user=None, details=None):
    ...
    if details:
        return {team: frappe.get_doc("dms Team", team) for team in teams}
...
def get_entity_with_permissions(entity_name):
    ...
    entity = frappe.db.get_value(
        "dms File", {"is_active": 1, "name": entity_name}, ENTITY_FIELDS + ["team"], as_dict=1
    )
    if not entity:
...
def get_shared_with_list(entity):
    ...
    if not frappe.has_permission(
        doctype="dms File", doc=entity, ptype="share", user=frappe.session.user
    ):
    ...
    owner = frappe.db.get_value("dms File", entity, "owner")
    permissions.insert(
        0,
        frappe.db.get_value(
...
def auto_delete_expired_perms():
    ...
    if expired_documents:
        ...
        def batch_delete_perms(docs):
            for d in docs:
                frappe.delete_doc("dms Permission", d.name)
