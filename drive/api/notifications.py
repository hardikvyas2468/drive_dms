...
def get_link(entity):
    ...
    return (
        entity.path if entity.is_link else f"/dms/{entity.team}/{type_.get(True)}/{entity.name}/"
    )
...
def get_notifications(only_unread):
    ...
    Notification = frappe.qb.DocType("dms Notification")
    fields = [
        Notification.name,
...
def mark_as_read(name=None, all=False):
    ...
    if all:
        frappe.db.set_value(
            "dms Notification", {"to_user": frappe.session.user, "read": False}, "read", True
        )
        return
    frappe.db.set_value("dms Notification", name, "read", True)
    return
...
def notify_mentions(entity_name, document_name):
    ...
    document = frappe.get_doc("dms Document", document_name)
...
def notify_share(entity_name, docperm_name):
    ...
    """
    entity = frappe.get_doc("dms File", entity_name)
    docshare = frappe.get_doc("dms Permission", docperm_name)
...
def create_notification(from_user, to_user, type, entity, message=None):
    ...
    """
    from dms.api.permissions import get_user_access
...
def send_share_email(to, message, link, team, type_):
    frappe.sendmail(
        recipients=to,
        subject=f"Frappe dms - {type_.capitalize()} Shared",
        template="drive_share",
        args={
            "message": message,
            "type": type_,
            "link": link,
            "team_name": frappe.db.get_value("dms Team", team, "title"),
        },
        now=True,
    )
