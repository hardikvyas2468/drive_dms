...
def create_tag(title, color="gray"):
    ...
    tag_exists = frappe.db.exists(
        {"doctype": "dms Tag", "owner": frappe.session.user, "title": title}
    )
    if tag_exists:
...
def get_entity_tags(entity):
    ...
    return map(
        lambda x: frappe.db.get_value("dms Tag", x.tag, ["name", "title", "color"], as_dict=1),
        entity.tags,
    )
...
def edit_tag(tag, title, color):
    ...
    """
    doc = frappe.get_doc("dms Tag", tag)
    doc.title = title
    doc.color = color
    doc.save()
...
def remove_tag(entity, tag=None, all=False):
    ...
    entity_doc = frappe.get_doc("dms File", entity)
...
def delete_tag(tag):
    ...
    :param tag: Tag name
    """
    EntityTag = frappe.qb.DocType("dms File Tag")
    query = frappe.qb.from_(EntityTag).select(EntityTag.name).where(EntityTag.tag == tag)
    result = query.run(as_dict=True)
    for i in result:
        frappe.delete_doc("dms File Tag", i.name)
    frappe.delete_doc("dms Tag", tag)
    return result
