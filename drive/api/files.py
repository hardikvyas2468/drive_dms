...
def get_thumbnail(entity_name):
    ...
    if not frappe.has_permission(
        doctype="dms File", doc=drive_file.name, ptype="write", user=frappe.session.user
    ):
        ...
        if not thumbnail_data:
            ...
            try:
                ...
                if drive_file.mime_type.startswith("text"):
                    ...
                    html = frappe.get_value("dms Document", drive_file.document, "raw_content")
...
def create_document_entity(title, personal, team, content, parent=None):
    ...
    drive_doc = frappe.new_doc("dms Document")
...
def stream_file_content(drive_file, range_header):
    ...
    :param entity_name: Document-name of the file whose content is to be streamed
    :param drive_file: dms File record object
    """
...
def call_controller_method(entity_name, method):
    ...
    """
    # broken
    drive_file = frappe.get_doc("dms File", frappe.local.form_dict.pop("entity_name"))
    if not drive_file:
...
def get_children_count(drive_file):
    children_count = frappe.db.count("dms File", {"parent_entity": drive_file, "is_active": 1})
