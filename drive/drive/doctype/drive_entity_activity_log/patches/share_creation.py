...
def create_activity_log(share):
    log = frappe.new_doc("dms Entity Activity Log")
...
def update_activity_log(log, share):
    title = frappe.db.get_value("dms File", share.share_name, ["title"])
    ...
    if share.everyone:
        ...
        log.new_value = share.user_name
    log.save()
    frappe.db.set_value("dms Entity Activity Log", log.name, "message", message)
    frappe.db.set_value("dms Entity Activity Log", log.name, "owner", share.owner)
    frappe.db.set_value("dms Entity Activity Log", log.name, "creation", share.creation)
