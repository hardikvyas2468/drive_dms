...
from dms.api.files import get_ancestors_of
from dms.utils.files import generate_upward_path
from dms.api.activity import create_new_activity_log
...
class DriveFile(Document):
    ...
    def on_trash(self):
        frappe.db.delete("dms Favourite", {"entity": self.name})
        frappe.db.delete("dms Entity Log", {"entity_name": self.name})
        frappe.db.delete("dms Permission", {"entity": self.name})
        frappe.db.delete("dms Notification", {"notif_doctype_name": self.name})
        frappe.db.delete("dms Entity Activity Log", {"entity": self.name})
    ...
    def after_delete(self):
        ...
        if self.document:
            frappe.delete_doc("dms Document", self.document)
    ...
    def move(self, new_parent=None, is_private=None):
        ...
        is_group = frappe.db.get_value("dms File", new_parent, "is_group")
        if not is_group:
        ...
        self.is_private = frappe.db.get_value("dms File", new_parent, "is_private")
        ...
        return frappe.get_value(
            "dms File", new_parent, ["title", "team", "name", "is_private"], as_dict=True
        )
    ...
    def copy(self, new_parent=None, parent_user_directory=None):
        ...
        if not parent_user_directory:
            parent_owner = (
                frappe.db.get_value("dms File", new_parent, "owner")
                if new_parent
                else frappe.session.user
            )
            # BROKEN - parent dir is team
            new_parent = new_parent or parent_user_directory.name
            parent_is_group = frappe.db.get_value("dms File", new_parent, "is_group")
            ...
            if self.name == new_parent or self.name in get_ancestors_of("dms File", new_parent):
        ...
        if self.is_group:
            ...
            drive_doc_content = frappe.db.get_list("dms Document", self.document, "content")

            new_drive_doc = frappe.new_doc("dms Document")
    ...
    def change_color(self, new_color):
        ...
        return frappe.db.set_value(
            "dms File", self.name, "color", new_color, update_modified=False
        )
    ...
    def permanent_delete(self):
        write_access = frappe.has_permission(doctype="dms File", doc=self, ptype="write")
        parent_write_access = frappe.has_permission(
            doctype="dms File",
            doc=frappe.get_value("dms File", self, "parent_entity"),
            ptype="write",
        )
    ...
    def share(self, user=None, read=None, comment=None, share=None, write=None, valid_until=""):
        ...
        if not permission:
            permission = frappe.new_doc("dms Permission")
        else:
            permission = frappe.get_doc("dms Permission", permission)
    ...
    def unshare(self, user=None):
        ...
        if perm_name:
            frappe.delete_doc("dms Permission", perm_name, ignore_permissions=True)


def on_doctype_update():
    frappe.db.add_index("dms File", ["title"])
