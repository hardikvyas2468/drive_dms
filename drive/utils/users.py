...
def mark_as_viewed(entity):
    ...
    if not frappe.has_permission(
        doctype="dms Entity Log", ptype="write", user=frappe.session.user
    ):
        return
    if entity.is_group:
        return
    ...
    entity_log = frappe.db.get_value(
        "dms Entity Log", {"entity_name": entity_name, "user": frappe.session.user}
    )
    if entity_log:
        frappe.db.set_value(
            "dms Entity Log", entity_log, "last_interaction", now(), update_modified=False
        )
        return
    doc = frappe.new_doc("dms Entity Log")
...
def add_comment(reference_name: str, content: str, comment_email: str, comment_by: str):
    ...
    exists = frappe.db.exists("dms File", reference_name)
    if not exists:
